...
export default function Step72SpouseSituations({ data }: StepProps) {
  const s = data.data.step72
  const update = (val: string[]) => data.updateStep('step72', (prev) => ({ ...prev, situations: val }))
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600">Selectionnez les situations qui s'appliquent a votre conjoint(e) pour 2025 :</p>
      <div className="grid grid-cols-1 gap-2">
        {SITUATIONS.map((sit) => (
          <label key={sit} className={`flex items-center gap-3 px-4 py-3 rounded-lg border text-sm cursor-pointer transition ${
            s.situations.includes(sit) ? 'bg-teal-50 border-teal-500 text-teal-700 font-medium' : 'border-gray-300 hover:bg-gray-50'
          }`}>
